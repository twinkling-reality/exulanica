...
109 tables are under FORCE row-level security keyed on ``current_workspace()``. The package-export receipt is
...
records are append-only so a later runtime write cannot rewrite why geometry was permitted.
