...
each id a route takes it makes a real object of that kind in the owner's workspace, from
`EXISTENCE_BUILDERS` in `tests/route_probes.py`, which is keyed by the id's address (the path up to
its placeholder), so a route taking a kind nobody can build fails by name. It sends every
combination of foreign and invented ids through an application connected as a non-superuser
runtime role, so row-level security applies as in a deployment, and it requires the owner's own
answer to differ, which shows the ids are real and the route looked them up. A planted route
