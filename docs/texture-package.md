...
for `exulanica_app`, `exulanica_ro`, `orimera_app` and `orimera_ro` with the same inline block 0042
uses. A trigger refuses every UPDATE and DELETE, even by the owner, and every INSERT by a role that
