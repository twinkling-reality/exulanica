...
describe('a saved world draws its inhabitants over its authored region', () => {
  it('builds exactly what an owned-district world built before it existed', async () => {
    ...
    // authored-region crowd was added, again when inhabitants became catalog people (drawn
    // as their far form while their containers load; this test registers no loader), and again
    // when the origin sky lost its 540 m scale (it is drawn around the eye): putting that one
    // scale back gives aa14da25. A district builds no authored crowd and sends the native
    // character runtime exactly the frames it did.
    expect(createHash('sha256').update(text).digest('hex'))
      .toBe('4292dbff1ee2e1e126f606c04c89eba83851f6634556827e3f2d044ad627473c');
