 ...
 * counts as covered.
 *
 *   - Dust is an uneven haze, a band of broad fractal noise at a third of the dust's strength, with
 *     flecks, the top of a fine value-noise field, only where the haze lies and more where it lies
 *     thick: dust settles unevenly and gathers, so a pane is never one even grey or an even speckle.
 *   - A rain streak is a thin line where a field stretched down the pane crosses its middle, cut
 *     into lengths by a second field, so streaks are narrow, broken and of unequal length, the way
 *     running water leaves them, rather than stripes the height of the tile.
 *   - Handling marks keep the upper part of broad fractal noise, faded in over a wide band so a
 *     mark has no edge: a smear, never a shape that repeats down a whole street.
 *
...
export const glazingManifest: ProceduralMakerManifest = {
  ...
  controls: [
    ...
    integer('dust_percent', 'wear', 'percent', [0, 100], 16, 'Dust',
      'The most light the fine dust speckle stops passing through, in percent.'),
    integer('dust_cells', 'detail', 'cells_per_tile', [16, 512], 160, 'Dust fineness',
      'How many dust flecks fit across one tile; more means finer dust.'),
    integer('streak_percent', 'wear', 'percent', [0, 100], 10, 'Rain streaks',
      'The most light the streaks left by running rain stop passing through, in percent.'),
    integer('streak_cells_across', 'detail', 'cells_per_tile', [4, 256], 40, 'Streak spacing',
      'How many streaks fit side by side across one tile.'),
    integer('streak_cells_along', 'detail', 'cells_per_tile', [1, 32], 2, 'Streak length',
    ...
    integer('smudge_cells', 'detail', 'cells_per_tile', [2, 64], 6, 'Mark size',
...
function pattern(recipe: Recipe): Pattern {
  ...
  return (x, y, out) => {
    const settled = band(fbm(x, y, 4, 4, hazeSeed, 3), 28000, 48000);
    const fleck = floorDiv(band(valueNoise(x, y, dustCells, dustCells, dustSeed), 50000, 60000) * settled, ONE);
    const dust = floorDiv(Math.max(fleck, floorDiv(settled, 3)) * dustPercent, 100);
    ...
    const lengths = band(valueNoise(x, y, streakAcross, streakAlong * 3, lengthSeed), 36000, 52000);
    const streak = floorDiv(floorDiv(line * lengths, ONE) * streakPercent, 100);
    const smudge = floorDiv(
      band(fbm(x, y, smudgeCells, smudgeCells, smudgeSeed, 3), 38000, 56000) * smudgePercent,
      100,
    );
