...
import { SocietyClient, type SocietyEvent, type SocietySnapshot } from '../society-api.js';
...
export interface LiveSocietyView {
  ...
  readonly status: 'idle' | 'loading' | 'ready' | 'stale' | 'unauthorized' | 'unavailable';
...
export function createLiveSociety(options: LiveSocietyOptions): LiveSociety {
  ...
  const client = options.client ?? new SocietyClient({ ...options.credentials, signal: abort.signal });
  let disposed = false;
  ...
  const checked = (snapshot: SocietySnapshot) => {
    if (snapshot.versionId !== options.versionId || snapshot.placeId !== options.placeId ||
  ...
  const api: LiveSociety = {
    ...
    advance: () => run(async () => {
      ...
      try { await accept(await client.advance(snapshot)); }
      catch (error) {
        if (!(error instanceof ApiError) || error.code !== 'stale_society_state') throw error;
        await accept(await client.read(options.versionId));
    ...
    afterAuthoredEdit: async () => {
      ...
      await run(async () => accept(await client.read(options.versionId),
        'Authored change saved. Advance to consume queued inputs. Restoring an object retains earlier simulation events.'));
    },
    inspect: subjectId => {
