...
export const REASON_WORDS: Readonly<Record<string, string>> = {
  ...
  no_enabled_affordance: 'there is nowhere to go yet',
  no_known_reachable_affordance: 'nowhere they know of can be reached',
  no_room_at_destination: 'every place they could use is taken',
  no_room_to_wait: 'there is no free spot to wait in',
  action_precondition_failed: 'they are not where it happens',
  validated_model_wait: 'they chose to wait',
  input_unavailable: 'this world cannot be walked right now',
};

const reasonWords = (code: string): string =>
  REASON_WORDS[code] ?? `of a reason this page has no words for (${code})`;
...
export function inhabitantWords(person: Inhabitant, rows: readonly PlaceRow[]): InhabitantWords {
  ...
  const place = (targetId: string | null | undefined) =>
    targetId ? named.get(targetId) ?? 'a place that is no longer there' : 'a free spot nearby';
  const who = person.display_name ?? 'Someone';
  const what = `A simulated ${person.role ?? 'inhabitant'}, invented for this world: not anyone you know, and nothing they do is a memory.`;
