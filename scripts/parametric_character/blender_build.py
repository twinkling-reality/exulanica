...
def build(config):
        ...
        if folder.is_dir():
            ...
            if link.exists() or link.is_symlink():
                if not link.is_symlink() or link.resolve() != folder.resolve():
                    raise ValueError(
                        "MPFB asset cache does not point to the verified source: " + folder.name
                    )
            else:
    ...
    macro["age"] = 0.4  # This initial family is an adult representation.
    ...
    for control in family["controls"]:
        ...
        for target in control.get("targets", []):
            if amount:
                path = (
                    Path(config["mpfb"])
                    / "src/mpfb/data/targets"
                    / (target + ("-incr" if amount > 0 else "-decr") + ".target.gz")
                )
    ...
    for side in ("l", "r"):
        TargetService.load_target(
            body,
            str(
                Path(config["mpfb"]) / f"src/mpfb/data/targets/eyes/{side}-eye-scale-decr.target.gz"
            ),
            weight=0.16,
        )
    ...
    for kind, name, label, color in [
        ("eyes", "low-poly", "Eyes", None),
    ...
    for obj, label, color in components:
        ...
        if color is None:
            ...
            bsdf.inputs["Roughness"].default_value = 0.62 if label == "Skin" else 0.72
            obj.data.materials.clear()
            obj.data.materials.append(material)
    ...
    mapping = {
        "mixamorig:" + k: v for k, v in mapping.items() if "mixamorig:" + k in rig.pose.bones
    }
    ...
    tpose = json.loads(
        (Path(config["mpfb"]) / "src/mpfb/data/poses/mixamo_unity_fk/t-pose.json").read_text()
    )
    ...
    for name in ["Idle", "Walk", "Run", "Interact"]:
        source_action = next(
            a for n, a in source_actions.items() if n == name or n.startswith(name + "_")
        )
            ...
            for bone in rig.pose.bones:
                ...
                if bone.name == "mixamorig:Hips":
                    ratio = (
                        ...
                        / source_rest[source_name].translation.z
                    )
                    bob = (
                        source_pose.translation.z - source_rest[source_name].translation.z
                    ) * ratio
    ...
    Path(config["metadata"]).write_text(
        json.dumps(
            {
                "height": height,
                "scale": scale,
                "floor": idle_floor,
                "colors": {label: color for _, label, color in components if color is not None},
                "clips": clips,
                ...
                "restJoints": {
                    b.name: list(rig.matrix_world @ b.head_local) for b in rig.data.bones
                },
            },
            indent=2,
        )
