...
def validate_recipe(value):
    ...
    for control in fields:
        ...
        elif isinstance(v, bool) or not isinstance(v, (int, float)) or not math.isfinite(v) or not control['min'] <= v <= control['max']:
...
class Builder:
    ...
    def generate(self, value):
        ...
        try:
            ...
            identity = preparation_identity(verify_inputs(self.source, self.blender, FAMILY_ROOT), FAMILY_ROOT)
            ...
            if result.exists():
                ...
                if asset.exists() and hashlib.sha256(asset.read_bytes()).hexdigest() == saved['descriptor']['asset']['contentSha256']:
                    return saved
            directory.mkdir(parents=True, exist_ok=True)
            config = dict(mpfb=str(self.source / 'mpfb2'), cache=str(self.source / 'mpfb-user'), recipe=recipe, family=FAMILY,
                assets=str(self.source / 'system-assets'), animation=str((FAMILY_ROOT / json.loads((FAMILY_ROOT / 'source-lock.json').read_text())['animationSource']).resolve()),
                output=str(directory / 'human.glb'), metadata=str(directory / 'metadata.json'))
            config_path = directory / 'config.json'
            config_path.write_text(json.dumps(config))
            with (directory / 'build.log').open('w') as log:
                subprocess.run([str(self.blender), '--background', '--factory-startup', '--python-exit-code', '1', '--python',
                    str(Path(__file__).with_name('blender_build.py')), '--', str(config_path)], stdout=log, stderr=subprocess.STDOUT,
                    check=True, timeout=150)
            if preparation_identity(verify_inputs(self.source, self.blender, FAMILY_ROOT), FAMILY_ROOT) != identity:
                raise ValueError('Preparation inputs changed during the build; result was not published')
            ...
            file = 'human-' + key + '.glb'
            descriptor = dict(asset=dict(assetKey='preview-human-' + key, mediaType='model/gltf-binary', contentSha256=digest, byteSize=len(data)),
                rigId='makehuman-mixamo-unity/v1', joints=list(dict.fromkeys(document['nodes'][i]['name'] for s in document['skins'] for i in s['joints'])),
                unitScale=metadata['scale'], standingHeight=metadata['height'], groundOffset=metadata['floor'], forwardYawDegrees=0,
                clips={g:dict(name='Human' + g.title(), metresPerSecond=metadata['clips'][g]['metresPerSecond'] * metadata['scale']) for g in ['idle','walk','run']},
                rootMotion=dict(mode='in-place'),materialSlots={m['name']:dict(materials=[m['name']]) for m in document['materials']},
                morphParameters={}, variantSlots={})
            look = dict(lookId='human-' + key, label='Editable human', creator='MakeHuman Community · Quaternius motion', license='CC0',
                file=file, descriptor=descriptor, defaultColors=metadata['colors'], recipe=recipe, familyId=FAMILY['familyId'], generated=True, preparationReceipt=identity)
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if re.fullmatch(r'/assets/human-[a-f0-9]{64}\.glb', self.path):
                ...
                self.wfile.write(data)
                return
            self.reply(200, FAMILY) if self.path == '/family' else self.reply(404, {'error':'Not found'})
        ...
        def do_POST(self):
            ...
            if origin and origin not in allowed_origins:
                return self.reply(403, {'error':'This builder belongs to the local character preview'})
            try:
                ...
                self.reply(503, {'error':'Body fitting failed. Your previous character is unchanged. Try less extreme proportions.'})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--allow-origin', action='append', type=loopback_origin, help='Repeat for each permitted local preview origin; replaces the 5192 defaults')
    args = parser.parse_args()
    create_server(Builder(args.blender.resolve(), args.source.resolve()), args.port, args.allow_origin or DEFAULT_ORIGINS).serve_forever()
