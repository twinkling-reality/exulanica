...
def verify_inputs(source, blender, family_root):
    ...
    if dirty.strip():
        raise ValueError(
            "MPFB preparation source is dirty; restore the pinned source before building"
        )
    ...
    if (
        version.splitlines()[0] != lock["blenderVersion"]
        or f"build hash: {lock['blenderBuildHash']}" not in version
    ):
        raise ValueError("Blender version or build does not match the preparation receipt")
    actual.update(
        mpfbCommit=head,
        blenderVersion=lock["blenderVersion"],
        blenderBuildHash=lock["blenderBuildHash"],
...
def preparation_identity(inputs, family_root):
    ...
    return {
        ...
        "scripts": {
            name: file_digest(scripts / name)
            for name in ["inputs.py", "blender_build.py", "preview_server.py"]
        },
    }
...
def cache_key(identity, recipe):
    return hashlib.sha256(
        json.dumps([identity, recipe], sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
