...
def component_materials(document, node_name):
    ...
    return {document['materials'][part['material']]['name']
...
def prepare() -> None:
    ...
    names = {'hoodie': 'Hoodie', 'casual': 'Everyday', 'casual-f': 'Relaxed', 'formal-f': 'Tailored'}
    for asset in manifest['assets']:
        receipt = json.loads((SOURCE / (asset['look_id'] + '.import.json')).read_text())
        descriptor = {
            'asset': {'assetKey': receipt['asset_key'], 'mediaType': 'model/gltf-binary',
                      'contentSha256': receipt['content_sha256'], 'byteSize': receipt['byte_size']},
            'rigId': asset['rig_profile'], 'joints': [joint['name'] for joint in asset['audit']['joints']],
            'unitScale': asset['frame']['unit_scale'], 'forwardYawDegrees': 0,
            'groundOffset': asset['frame']['idle_floor_offset_metres'],
            'standingHeight': asset['frame']['height_metres'],
            'clips': {name: {'name': asset['clips'][name]['name'], 'metresPerSecond': asset['clips'][name]['nominal_speed_mps']}
                      for name in ['idle', 'walk', 'run']},
            'rootMotion': {'mode': 'in-place'},
            'materialSlots': {name: {'materials': [slot['material_name']]} for name, slot in asset['material_slots'].items()},
        ...
        if len(data) != reference['byteSize'] or hashlib.sha256(data).hexdigest() != reference['contentSha256']:
        ...
        colors = {m['name']: hex_color(m.get('pbrMetallicRoughness', {}).get('baseColorFactor', [1, 1, 1, 1]))
                  for m in gltf['materials']}
        item['label'] = names.get(item['lookId'], item['lookId'])
        item['defaultColors'] = {slot: colors[spec['materials'][0]]
        ...
        garment_materials = set().union(*(component_materials(gltf, node)
            for slot, node in asset['slots'].items() if slot != 'head_hair'))
        item['variationSlots'] = sorted(slot for slot, spec in descriptor['materialSlots'].items()
        ...
        eye = asset['first_person_reference']['eye_center_model_metres'] or [0, asset['frame']['height_metres'] * .91, .20]
        item['gesture'] = {'file': item['file'], 'descriptor': {
            'character': descriptor, 'clip': asset['clips']['interact']['name'],
            'durationSeconds': asset['clips']['interact']['duration_seconds'], 'eye': eye,
