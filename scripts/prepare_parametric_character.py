...
def install(source):
    ...
    repo = source / 'mpfb2'
    if not repo.exists():
        subprocess.run(['git', 'clone', '--filter=blob:none', '--no-checkout', lock['mpfbRepository'], str(repo)], check=True)
        subprocess.run(['git', '-C', str(repo), 'checkout', '--detach', lock['mpfbCommit']], check=True)
...
if __name__ == '__main__':
    ...
    publish_default(FAMILY_ROOT / 'human-default.glb', ROOT / 'web/packages/app/public/fixtures/characters')
    print('Prepared the default editable human. Run prepare_character_preview.py to refresh the catalog.')
