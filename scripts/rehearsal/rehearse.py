...
3.  Takes the launcher's record of that build (its script and index hashes, and the hash of the page
    ...
    this script then starts the worker main runs in production, the command the step list's
    ``runtime.derivative_worker`` names, from that worktree's environment, over the run's
    workspace and database, with the environment the step list gives it (its depth model among
    it) and, with ``--model-env``, the key through the same child-process handoff. It waits for
    the worker's startup event and stops it before the launcher stops the API.
...
class Run:
    ...
    def __init__(self, arguments: argparse.Namespace) -> None:
        ...
        self.worker_record: dict[str, Any] = {}
    ...
    def worker_up(self) -> None:
        """Start the derivative worker main runs in production, as the step list configures it.
        ...
        is added to a clean one; the key, when the run has a model, comes through the same handoff
        as the API's, with the run's bound, so this process never holds it either.
        """
        assert self.state is not None
        configured = self.steps["runtime"]["derivative_worker"]
        ...
        command = [str(program), "--name", configured["name"]]
        if self.model_configured:
            environment["EXULANICA_BUDGET_USD"] = str(self.bound)
            command = [*self.key_handoff(), *command]
        log_file = self.out / "derivative-worker.log"
        ...
        self.worker_record = {
            ...
            "model": self.model_configured,
            "log": "derivative-worker.log",
        }
        ...
        while time.monotonic() < deadline:
            ...
            if any(e.get("event") == "startup" for e in events):
                self.worker_record["startup_seconds"] = round(time.monotonic() - started, 1)
                return
            if any(e.get("event") == "startup_failed" for e in events) or self.worker.poll():
                break
            time.sleep(1)
        tail = scrub(log_file.read_text(errors="replace")[-800:])
        raise Refused(f"the derivative worker did not start: {tail}")
    ...
    def worker_down(self) -> None:
        ...
        if self.worker.poll() is None:
            ...
            try:
                self.worker.wait(timeout=self.steps["runtime"]["derivative_worker"]["stop_seconds"])
            except subprocess.TimeoutExpired:
                os.killpg(self.worker.pid, signal.SIGKILL)
                self.worker.wait()
        self.worker_record["exit"] = self.worker.returncode
        self.worker_record["events"] = sorted(
            {str(e.get("event")) for e in worker_events(self.out / "derivative-worker.log")}
        )
    ...
    def write_result(self) -> dict[str, Any]:
        ...
        run = {
            ...
            "runtime": {
                ...
                "derivative_worker": self.worker_record
