...
from exulanica.environment.city_context import (
...
class AnswerView(BaseModel):
    ...
    #: it stands for. A person's saved name is never sent to a hosted model, and a place's only
    #: under the account holder's right for that place and the composer, so a composed answer names
...
def ask(
    ...
    if body.city_context is not None:
        ...
        if plan is None:
            return AnswerView(
                answer=answer_without_a_bridge(city_clause),
                plan=None,
                selection=None,
                citations={},
                abstained=None,
...
def _city_selection(
    ...
    *,
    ...
    """The city selection resolved, each refusal answered with its HTTP status."""
    try:
        return resolve_city_selection(
            connection,
    ...
    except CitySelectionRefused as exc:
