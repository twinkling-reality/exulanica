...
def test_a_scene_in_another_workspace_is_not_readable(published, repository, ingest_spine):
    ...
    assert (
        world_read_bundle(other.connection, elsewhere, scene_id, store, world_id=FIXTURE_WORLD_ID)
        is None
    )
...
def test_an_unknown_scene_is_not_readable(published, repository):
    ...
    assert (
        world_read_bundle(
            repository.connection, repository.workspace_id, absent, store, world_id=FIXTURE_WORLD_ID
        )
        is None
    )
...
def test_a_bundle_without_a_store_still_reports_honestly(published, repository):
    ...
    envelope = world_read_bundle(
        repository.connection, repository.workspace_id, scene_id, None, world_id=FIXTURE_WORLD_ID
    )
    assert envelope is not None
    bundle = envelope["bundle"]
