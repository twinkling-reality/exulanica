...
def test_v4_persists_a_place_sized_population_with_receipts_reload_and_replay(living):
    ...
    assert api.post(path, body).json() == created  # idempotent create
    assert api.post(path, body | {"profile": "exulanica-society/v2"}).status_code == 409
    for _ in range(3):
    ...
    assert api.get(path).json() == result
    stale = api.post(
        api.in_world(route + "/steps"), {"base_tick": 3, "base_state_sha256": "0" * 64}
    )
...
def test_v4_refuses_a_tampered_stored_state_and_keeps_older_population_bounds(living):
    ...
    held = api.get(api.in_world(route)).json()
