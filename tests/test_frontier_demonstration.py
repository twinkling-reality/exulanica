...
@pytest.mark.parametrize("screening_kind", ["synthetic", "observation-only"])
def test_frontier_demonstration_runs_every_gate_without_fabricating_reconstruction(
    repository, tmp_path, screening_kind
):
    ...
    # Fixture authority is prepared explicitly, outside the ordinary demonstration path.
    store = LocalContentAddressedStore(tmp_path / "data" / "blobs")
    intake_pipeline = PhotoIngestPipeline(repository, store)
    for path in sorted(photo_dir.iterdir()):
        intake = intake_pipeline.ingest_intake(path.read_bytes(), filename=path.name)
        if screening_kind == "synthetic":
            authorization = authorize_synthetic_capture(
                repository,
                capture_id=intake.capture_id,
                actor=actor,
                generator_manifest={"profile": "exulanica.test-frontier/v1"},
                authorization_scope={"purpose": "automated test with generated photographs"},
            )
            ...
            authorization = authorize_personal_capture(
                ...
                capture_id=intake.capture_id,
                actor=actor,
                account_authority_basis="generated fixture exercising personal admission",
                authorization_scope={"purpose": "automated observation-only permission test"},
                purpose="test observation-only permission with generated bytes",
            )
            record_person_detection_screening(
                repository,
            ...
            # The detection receipt names no model, so the account holder also names every model
            # the vision role can reach; without these rights nothing is sent.
            for identity in HostedVisionDouble().model_handoff.identities:
                grant_model_right(
                    repository,
                    capture_id=intake.capture_id,
                    authorization_id=authorization.authorization_id,
                    identity=identity,
                    destination=HostedVisionDouble().model_handoff.destination,
                    granted_by=actor,
                    purpose="test observation-only permission with generated bytes",
                    valid_until=dt.datetime.now(dt.UTC) + dt.timedelta(hours=1),
                )
    vision = HostedVisionDouble()
    original_bytes = {p.name: p.read_bytes() for p in photo_dir.iterdir()}
    ...
    assert {p.name: p.read_bytes() for p in photo_dir.iterdir()} == original_bytes
...
def test_supplied_photos_cannot_grant_themselves_synthetic_screening(repository, tmp_path):
    photo_dir = tmp_path / "photos"
    photo_dir.mkdir()
    write_photo(photo_dir, "a.jpg", make="Exulanica-A")
    write_photo(photo_dir, "b.jpg", make="Exulanica-B")
    originals = {path.name: path.read_bytes() for path in photo_dir.iterdir()}
    manifest = _manifest(tmp_path, photo_dir, repository.workspace_id, uuid.uuid4())
    vision = CountingVisionModel()
    with pytest.raises(FrontierDemonstrationError, match="privacy_authorization"):
        run_frontier_demonstration(
            repository.connection,
            manifest=manifest,
            photo_dir=photo_dir,
    ...
    assert originals == {path.name: path.read_bytes() for path in photo_dir.iterdir()}
    for table in (
        "capture",
        ...
        assert (
            repository.connection.execute(f"select count(*) as n from {table}").fetchone()["n"] == 0
        )
